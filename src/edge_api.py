...
class EdgeAPI(object):
    def __init__(self, node) -> None:
        self.api_host = node.remote_api['host']
        self.api_ver = node.remote_api['apiver']
        self.node_id = node.node_id
        self.access_key = node.access_key
        ...
        self.__request_timeout = node.remote_api.get(
    ...
    def get_node(self) -> dict:
        ...
        if status_code == 200:
            ...
            logger.error(
                f"Error {status_code} returned from metadata API request")
    ...
    def get_config(self) -> dict:
        ...
        if status_code == 200:
            if rtn.get('config'):
                ...
                logger.error(
                    'API call successful but response did not include a config payload')
                return None
        else:
            logger.error(
                f"Error {status_code} returned from config API request")
            logger.info(f"API response: {rtn}")
            return None
    ...
    def get_command(self) -> str:
        ...
        if status_code == 200:
            ...
            logger.error(
                f"HTTP Error {status_code} returned from command API request")
            logger.info(f"API response: {rtn}")
            return None
    ...
    def get_upload_url(self) -> str:
        ...
        if status_code == 200:
            if rtn.get('upload_url'):
                logger.info(
                    f"Obtained upload URL {rtn['upload_url']} from API")
    ...
    def post_env_scan(self, scan_result: dict) -> bool:
        status_code, rtn = self.__post_request(
            '/env_scan', payload=scan_result)
        if status_code in [200, 204]:
            logger.info("Successfully submitted environment scan")
            return True
        else:
            logger.error(
                f"HTTP Error {status_code} while trying to to submit environment scan to API")
            return False

    def __get_request(self, endpoint: str, params: dict = None) -> dict:
        r = self.__make_http_request(
            self._base_url + endpoint, 'GET', None, params)
        return self.__parse_response(r)

    def __post_request(self, endpoint: str, payload: dict = None, params: dict = None) -> dict:
        r = self.__make_http_request(
            self._base_url + endpoint, 'POST', payload, params)
        return self.__parse_response(r)
    ...
    def __make_http_request(
        ...
        try:
            ...
            logger.error(
                f"Connection error {e} while doing {method} request to {url}")
        except requests.exceptions.Timeout as e:
            logger.error(
                f"Timeout error {e} while doing {method} request to {url}")
        except Exception as e:
            logger.exception(
                f"Exception {e} while doing {method} request to {url}")
        ...
        if retry_count < MAX_REQUEST_RETRIES:
            retry_count += 1
            logger.info(
                f"Will retry (#{retry_count}/{MAX_REQUEST_RETRIES}). First sleeping {REQUEST_HOLDOFF} s.")
    ...
    def __parse_response(r: requests.Response) -> (int, dict):
        ...
        try:
            ...
            if r.status_code != 204:
                logger.error(
                    f"Response from API: {r.text}. Cannot be parsed as JSON")
