...
def parse_datagram_response(response: bytes) -> dict:
    ...
    while position < data_length:
        ...
        if datatype == 'actual':
            value = int.from_bytes(response[position+4:position+8], byteorder='big')
            position+=8
            if measurement in sma_channels.keys():
                values[sma_channels[measurement][0]]=value/sma_units[sma_channels[measurement][1]]
                values[sma_channels[measurement][0]+'unit']=sma_channels[measurement][1]
        # counter values
        elif datatype=='cumulative':
            value = int.from_bytes(response[position+4:position+12], byteorder='big')
            position+=12
            if measurement in sma_channels.keys():
                values[sma_channels[measurement][0]+'counter'] = value/sma_units[sma_channels[measurement][2]]
                values[sma_channels[measurement][0]+'counterunit'] = sma_channels[measurement][2]
